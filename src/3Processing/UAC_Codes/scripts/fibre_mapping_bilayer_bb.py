...
my_parser.add_argument(
    "bilayer_projection_distance_bb",
    metavar="value",
    type=int,
    help="Bilayer projection distance for Bachmann's Bundle",
)
my_parser.add_argument(
    "labels_tag",
    metavar="value",
...
bilayer_projection_distance_bb = args.bilayer_projection_distance_bb
...
print(base_dir)
print(fibre_epi_dir)
Pts_XYZ_0 = utils.read_pts(fibre_bb_dir + "Labelled")
Elems_XYZ_0 = utils.read_elem(fibre_bb_dir + "Labelled")
...
Scalar_Label_BB = np.loadtxt(base_dir + "MappedScalar_BB_LA.dat")
...
scalfac = bilayer_projection_distance
...
# now project outwards for BB
x_norm = []
y_norm = []
z_norm = []

scalfac = bilayer_projection_distance_bb
for loop in pointNormals:
    x_norm.append(np.multiply(loop[0] / np.sqrt(loop[0] ** 2 + loop[1] ** 2 + loop[2] ** 2), scalfac))
    y_norm.append(np.multiply(loop[1] / np.sqrt(loop[0] ** 2 + loop[1] ** 2 + loop[2] ** 2), scalfac))
    z_norm.append(np.multiply(loop[2] / np.sqrt(loop[0] ** 2 + loop[1] ** 2 + loop[2] ** 2), scalfac))
...
x_list_All = np.concatenate([xlist_1, xlist_Epi, xlist_BB])
y_list_All = np.concatenate([ylist_1, ylist_Epi, ylist_BB])
z_list_All = np.concatenate([zlist_1, zlist_Epi, zlist_BB])
...
Elems_2 = np.concatenate([Elems_Endo, Elems_Epi, Elems_BB, Elems_Ln])
