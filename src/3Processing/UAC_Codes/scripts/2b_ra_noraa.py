...
Pts2D = []
pts_2D = utils.read_pts(base_dir + "Labelled_Coords_2D_Rescaling_N3")
xlist_2D = pts_2D[:, 0]
ylist_2D = pts_2D[:, 1]
...
for inds in range(0, len(Pts_Labelled), 1):
    if ylist_2D[inds] < 0.5:
        ...
        Index = Index[0]
        Pts2D.append([1 - ylist_2D[inds], xlist_2D[inds], 0])
        counter2 = counter2 + 1
...
utils.write_carp(spts, selems, None, None, base_dir + "Labelled_Coords_2D_Rescaling_v3_C")
