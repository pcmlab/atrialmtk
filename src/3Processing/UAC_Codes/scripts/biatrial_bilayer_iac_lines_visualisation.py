...
for ind1 in range(len(CS_epi)):
    P1 = CS_epi[ind1]
    P_x_csepi.append(xlist[P1[0]])
    P_y_csepi.append(ylist[P1[0]])
    P_z_csepi.append(zlist[P1[0]])

PTS_CS_epi = [P_x_csepi, P_y_csepi, P_z_csepi]
PTS_CS_epi = list(zip(*PTS_CS_epi))
...
# 1. find rim of CS
pts_cs, elems_cs, *_ = utils.read_carp(base_dir_ra, mesh_name_ra, return_surface=False)

print("Reading seeds file")
pts_region = np.loadtxt(base_dir_ra + seed_region, delimiter=",") * scale_factor

size = 3
pts_pv = [None] * size
nodes_pv = [None] * size
for i in range(size):
    pts_pv[i], nodes_pv[i] = utils.pv_boundary(i, elems_cs, pts_cs)
...
# repeat for lipv
tot_pts_sub = utils.get_sub_surface(base_dir_bi, pts_cs, elems_cs, nodes_ripv, return_pts=True)

print(len(tot_pts_sub))
print(len(pts_cs))
...
for loop in pts_cs:
    xlistcs.append(loop[0])
    ylistcs.append(loop[1])
    zlistcs.append(loop[2])
...
for ind1 in range(len(nodes_ripv)):
    P1 = nodes_ripv[ind1]
    P_x_cs.append(xlistcs[P1])
    P_y_cs.append(ylistcs[P1])
    P_z_cs.append(zlistcs[P1])


PTS_CS = [P_x_cs, P_y_cs, P_z_cs]
PTS_CS = list(zip(*PTS_CS))


# 2. join each node to closest node on LA_epi
neigh_RA = NearestNeighbors(n_neighbors=1)
neigh_RA.fit(PTS_CS_epi)
neigh_LA = NearestNeighbors(n_neighbors=1)
neigh_LA.fit(PTS_LA_epi)

D_Store = []

for ind in range(len(P_x_cs)):
    _, Closest_RA = neigh_RA.kneighbors([PTS_CS[ind]], return_distance=True)
    LA_D1, _ = neigh_LA.kneighbors([PTS_CS_epi[Closest_RA[0][0]]], return_distance=True)
    D_Store.append(LA_D1[0][0])

print(D_Store)
List_To_Check = np.argsort(D_Store)
ToKeepLength = round(len(List_To_Check) / 2)
print(List_To_Check)
...
for indover in range(ToKeepLength):
    ind = List_To_Check[indover]
    _, Closest_RA = neigh_RA.kneighbors([PTS_CS[ind]], return_distance=True)
    _, Closest_LA1 = neigh_LA.kneighbors([PTS_CS_epi[Closest_RA[0][0]]], return_distance=True)
    Elems_Ln.append(Element("Ln", [CS_epi[Closest_RA[0][0]][0], LA_epi[Closest_LA1[0]][0][0]], 40))
